...
人気のnoteクリエイターです。ペンネームは「海外経済リサーチ室」。
...
■ TITLE
- ★「今日のフォーカス記事」を軸にした具体的なタイトルにすること
- 週次まとめ形式（「今週の〜」）は絶対に使わない      
- 「今日」「速報」「〜が判明」「〜の真相」を積極的に使う 
- 最大40文字の日本語タイトル
...
def build_note_prompt(market_data: dict, intel: dict) -> str:
    ...
    day = intel.get("day_label", datetime.now().strftime("%Y-%m-%d"))
    ...
    top_block = (
        f"ソース    : {top_item.get('source', '')}\n"
        f"タイトル  : {top_item.get('title', '')}\n"
        f"概要      : {top_item.get('summary', '')[:300]}\n"
        f"センチメント: {top_item.get('sentiment', 'N/A')}"
    ) if top_item else "（本日のトップ記事なし）"
...
---
上記の「今日のフォーカス記事」をメインテーマにして、
日本人個人投資家向けのnote記事を作成してください。
「今週の〜」という表現は使わず、「今日」「本日」「現在」を使ってください。
**■ TITLE** から始めてください。
"""
    return prompt
...
def generate_note_article(market_data: dict, intel: dict) -> dict:
    ...
    # generate_note_jp.py の generate_note_article() 内
# full_text = response.text の直後から result = {...} までを丸ごと差し替え
    ...
    for i, line in enumerate(lines):
        ...
        # ── タイトル抽出 ─────────────────────────────
        # 対応形式: "■ TITLE", "**TITLE**", "TITLE:", "# TITLE" など
        if not title and re.search(r'(■\s*TITLE|\*\*TITLE\*\*|^TITLE[：:]|^#\s*TITLE)', s, re.IGNORECASE):
            # 同じ行に値がある場合（例: "■ TITLE\nFRB声明..." → 次の行）
            cleaned = re.sub(r'(■\s*TITLE|\*\*TITLE\*\*|TITLE[：:]?)', '', s, flags=re.IGNORECASE).strip(' :()*"\'　：')
            if cleaned:
                title = cleaned
            else:
                # 次の行が値
                for next_line in lines[i+1:i+3]:
                    ns = next_line.strip()
                    if ns and not re.search(r'(SUBTITLE|■|##)', ns, re.IGNORECASE):
                        title = ns.strip(' :()*"\'　：')
                        break

        # ── サブタイトル抽出 ─────────────────────────
        elif not subtitle and re.search(r'(■\s*SUBTITLE|\*\*SUBTITLE\*\*|^SUBTITLE[：:])', s, re.IGNORECASE):
            cleaned = re.sub(r'(■\s*SUBTITLE|\*\*SUBTITLE\*\*|SUBTITLE[：:]?)', '', s, flags=re.IGNORECASE).strip(' :()*"\'　：')
            if cleaned:
                subtitle = cleaned
            else:
                for next_line in lines[i+1:i+3]:
                    ns = next_line.strip()
                    if ns and not re.search(r'(■|##|ヘッダー)', ns, re.IGNORECASE):
                        subtitle = ns.strip(' :()*"\'　：')
                        break
    ...
    # ── フォールバック ────────────────────────────────
    if not title:
        title = f"今週の海外市場レポート｜{intel.get('week_label', '')}"
    if not subtitle:
        subtitle = "FRB・決算・円相場から読み解く今週の投資判断"
    if not hashtags:
        hashtags = "#米国株 #新NISA #FRB #資産運用 #グローバル投資ラボ週報"

    # ── BODY（最初の##見出しから末尾まで）────────────────
    body_start = None
    skip_patterns = re.compile(r'(TITLE|SUBTITLE|ヘッダー画像|画像テキスト|ハッシュタグ)', re.IGNORECASE)
    for i, line in enumerate(lines):
        s = line.strip()
        if s.startswith("## ") and not skip_patterns.search(s):
            body_start = i
            break
    ...
    if body_start is not None:
        ...
        body = "\n".join(
            ln for ln in lines
            if not re.search(r'(■\s*TITLE|■\s*SUBTITLE|\*\*TITLE|\*\*SUBTITLE|【画像テキスト】|ハッシュタグ)', ln, re.IGNORECASE)
        ).strip()

    result = {
        "title":        title,
        "subtitle":     subtitle,
        "hashtags":     hashtags,   # ← 追加
        "body":         body,
